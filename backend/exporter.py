"""Generate 庫存總表 Excel — Sophie's 6/17 simplified layout (RC/華強 merged, 今日投資總結, 券商)"""
import tempfile
...
# Map detailed entity → display group (RC+私銀RC merge into RC, etc.)
...
def _cell(ws, row, col, value=None, font_size=20, bold=False,
          ...
          num_fmt=None, color='000000', border=False, fill=None):
...
def generate_excel() -> str:
    ...
    # ── Column widths (from Sophie's 新版本 reference) ──────────────────────────
    col_w = {
        'A': 11, 'B': 22, 'C': 26.4, 'D': 24.9, 'E': 24, 'F': 24,
        'G': 23.9, 'H': 20, 'I': 26.4, 'J': 23.9, 'K': 21.7, 'L': 23.3,
        'M': 17.7, 'N': 24, 'O': 25.7, 'P': 21.7, 'Q': 19.7,
        'R': 19.7, 'S': 9.1
    }
    ...
    # ── Row 2: 今日投資總結 (yellow) ──────────────────────────────────────────
    today_txns = (Transaction.query
                  .filter(Transaction.trade_date == date.today())
    ...
    if today_txns:
        lines = []
        for t in today_txns:
            direction = '買進' if t.shares > 0 else '賣出'
            zhang = abs(t.shares) / 1000
            zhang_str = f'{zhang:.3f}'.rstrip('0').rstrip('.')
            entity_disp = ENTITY_GROUP.get(t.entity, t.entity)
            lines.append(
                f'{entity_disp} {t.broker} {direction} {t.security_name or t.security_code} '
                f'{zhang_str}張 @ {t.price:,.2f}'
            )
        summary_text = '今日投資總結：\n' + '\n'.join(lines)
    else:
        summary_text = '今日投資總結：（今日無投資相關交易）'

    ws.merge_cells('A2:L2')
    _cell(ws, 2, 1, summary_text, font_size=18, bold=False, align='left',
          valign='top', wrap=True, fill='FFFF00')
    ws.row_dimensions[2].height = max(114.0, 24 * (len(today_txns) + 1))
    ...
    headers_r3 = [('A3','A4','股票\n代號'), ('B3','B4','股票名稱'),
                  ('C3','C4','券商'), ('D3','D4','市價'),
                  ('E3','G3','RC'), ('H3','J3','華強'), ('K3','L3','未實現損益')]
    for start, end, label in headers_r3:
        if start != end:
            ws.merge_cells(f'{start}:{end}')
        sc = ws[start]
        sc.value     = label
        fill_color   = 'FFFF00' if start == 'C3' else None
        sc.font      = Font(name='微軟正黑體', size=20, bold=True)
        sc.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
        sc.border    = _thin_border()
        if fill_color: sc.fill = PatternFill('solid', start_color=fill_color)
    ws.row_dimensions[3].height = 36

    # ── Row 4: Sub-headers ────────────────────────────────────────────────────
    sub_headers = {5:'張數', 6:'成本', 7:'金額', 8:'張數', 9:'成本', 10:'金額',
                   11:'RC', 12:'華強'}
    for col_i, label in sub_headers.items():
        c = ws.cell(4, col_i, label)
        c.font      = Font(name='微軟正黑體', size=20, bold=True)
        c.alignment = Alignment(horizontal='center', vertical='center')
        c.border    = _thin_border()
    ws.row_dimensions[4].height = 51

    # ── Data rows ─────────────────────────────────────────────────────────────
    # Aggregate positions: merge RC+私銀RC and 華強+私銀華強
    ...
    # Determine 券商 per stock from transaction history (most recent broker used)
    broker_by_code = {}
    ...
    for idx, (code, data) in enumerate(sorted(by_code.items())):
        ...
        c3 = ws.cell(row, 3, broker)
        c3.font      = Font(name='微軟正黑體', size=20, bold=True)
        c3.alignment = Alignment(horizontal='center', vertical='center')
        c3.border    = _thin_border()
        c3.fill      = PatternFill('solid', start_color='FFFF00')
    ...
    # ── 損益 table (RC/華強 only, no private split) ─────────────────────────────
    ...
    _cell(ws, row, 3, '合計', font_size=16, bold=True, align='center', border=True)
    ws.row_dimensions[row].height = 50.1
    row += 1

    for col_i, label in [(3,'RC'), (4,'華強')]:
    ...
    # Sum realized across merged groups (RC+私銀RC, 華強+私銀華強)
    ...
    for ent in DISPLAY_ENTITIES:
        ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=2)
        _cell(ws, row, 1, ent, font_size=16, bold=True, align='left', border=True)
        ws.merge_cells(start_row=row, start_column=3, end_row=row, end_column=4)
        bal = round(entity_balance.get(ent, 0))
        c = ws.cell(row, 3, bal)
