...
def create_app():
    ...
    def get_realized_pnl_ledger():
        """Full audit-trail ledger for 已實現損益 tab."""
    ...
    def add_pnl_adjustment():
        ...
        adj = PnlAdjustment(
            ...
            net_amount    = float(data['net_amount'])    if data.get('net_amount')    not in (None,'') else None,
            cost_basis    = float(data['cost_basis'])    if data.get('cost_basis')    not in (None,'') else None,
            realized_pnl  = float(data['realized_pnl']),
            original_pnl  = float(data['original_pnl']) if data.get('original_pnl') not in (None,'') else None,
    ...
    def update_pnl_adjustment(adj_id):
        ...
        if 'security_code' in data:   adj.security_code= data['security_code'] or None
        if 'security_name' in data:   adj.security_name= data['security_name'] or None
        if 'net_amount'    in data:   adj.net_amount   = float(data['net_amount'])   if data['net_amount']   not in (None,'') else None
        if 'cost_basis'    in data:   adj.cost_basis   = float(data['cost_basis'])   if data['cost_basis']   not in (None,'') else None
...
def _migrate_add_pnl_adjustments():
    """Create pnl_adjustments table and ensure new columns exist."""
    try:
        db.session.execute(db.text("""
        ...
        for col in [
            ...
            db.session.execute(db.text(col))
        db.session.commit()
    except Exception:
        db.session.rollback()
